...
- Initial emissions factors (e.g. `HEMCO_Config:66`) are arbitrary; these will be adjusted a posteriori to match simulation outputs to observations
- Same mass emitted for each tracer size *except* for ocean PST1 tracer (0.3 µm diameter), which has about 1.65% the mass of the other ocean tracers. This is because PST1 emissions are computed from accumulation-mode sea salt emissions (SALA) while all other tracers are computed from coarse-mode sea salt emissions (SALC).
...
Each source directory configures four timeperiods:

- `spinup`: spinup simulation from 2017-11-01 to 2018-01-01
- `2018-01-01_2019-01-01`: simulation for 2018-01-01 to 2019-01-01
- `2019-01-01_2020-01-01`: simulation for 2019-01-01 to 2020-01-01
- `2020-01-01_2021-01-01`: simulation for 2020-01-01 to 2021-01-01

Each timeperiod's `Restarts` directory contains a symlink to the restart file produced by the preceding simulation.
